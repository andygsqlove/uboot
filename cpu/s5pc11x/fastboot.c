...
fastboot_ptentry ptable_default[] =
{
	...
	{
		.name     = "system",    // 0X82 --> 130 M
		.start    = 0xE00000,
		.length   = 0x8200000,//0x6E00000, // changed from 110M to 130M
		.flags    = FASTBOOT_PTENTRY_FLAGS_WRITE_YAFFS
	},
	{
		.name     = "cache",         //  60 M
		.start    = 0x9000000,//0x7C00000, // changed from 110M to 130M
		.length   = 0x3c00000,//0x500 0000,
		.flags    = FASTBOOT_PTENTRY_FLAGS_WRITE_YAFFS
	},

	{
		.name     = "userdata",
		.start    = 0xCC00000,     // 204 M
		.length   = 0,
		.flags    = FASTBOOT_PTENTRY_FLAGS_WRITE_YAFFS
	}
#endif
...
int keypad_test(void)
{
	...
	int iTmp = 1;
	int iDelayCounter = 5;
	printf(" WILL ENTER keypad test LOOP\n");
		...
		if (!iTmp)
{
	...
	printf("enter keypad test loop\n");

	int gpio_value = 0xBF;
	int keymask[8];
	int cval = 0,rval = 0 ,col; 
	/*key pad GPIO*/
	/*set GPH2CON[0,1] to input*/
	writel((readl(ELFIN_GPIO_BASE+GPH2CON_OFFSET)&(~0xFF)), ELFIN_GPIO_BASE+GPH2CON_OFFSET);
	/*GPH2PUD[0,1] power up enable*/
	writel(readl(ELFIN_GPIO_BASE+GPH2PUD_OFFSET)&(~0xF)|0xA, ELFIN_GPIO_BASE+GPH2PUD_OFFSET);
	/*GPH2DAT[0,1] pull up*/
	//writel(readl(ELFIN_GPIO_BASE+GPH2DAT_OFFSET)&(0x3), ELFIN_GPIO_BASE+GPH2DAT_OFFSET);
	while(1)
	{
		gpio_value = (readl(ELFIN_GPIO_BASE+GPH2DAT_OFFSET)&0x3);
		/*find the row input */
		if(gpio_value == 0x1 || gpio_value == 0x2)
		{
			printf("value= 0x%x\n",gpio_value);
			//gpio_value = 0x3;
			//writel(cval, ELFIN_GPIO_BASE+GPH2DAT_OFFSET);
			udelay(900);
		}
		else 
		{
...
#if 0
		gpio_value = (readl(ELFIN_GPIO_BASE+GPH3DAT_OFFSET)&0xFF);
		/*find the row input */
		if(gpio_value != 0x9F)
		{
			...
			for(col =0; col< 8; col++)
			{
				...
				cval = 0x3fff & ~((1<<col) |(1 << col +8));
				writel(cval, ELFIN_GPIO_BASE+GPH2DAT_OFFSET);
				udelay(900);
				rval = ~(readl(ELFIN_GPIO_BASE+GPH3DAT_OFFSET)) & ((1<<8)-1);			
				keymask[col] = rval;
				printf("keymask[%d] = 0x%x \n",col,keymask[col] );
			}
		}
		writel(readl(ELFIN_GPIO_BASE+GPH2DAT_OFFSET) &~0xffff, ELFIN_GPIO_BASE+GPH2DAT_OFFSET);	
#endif	
	}
	return 1;	
}
...
int recovery_preboot(void)
{
	...
	int value = (readl(INF_REG_BASE+INF_REG5_OFFSET) & 0xF);
	//printf("INF_REG5_OFFSET value = %d\n",value);
	
	if(value)
	{
		printf("got recovery command\n");
		writel(readl(INF_REG_BASE+INF_REG5_OFFSET)&(~0x1),INF_REG_BASE+INF_REG5_OFFSET);
		/*got the factory reset signal, format data as ext3 cache as ext4*/
		if(sd_format_mmcs())
		{
			printf("sd_format failed\n");
			return -1;
		}
		return 0;
	}
	else
	{
		...
		while((readl(ELFIN_GPIO_BASE+GPH2DAT_OFFSET)&0x3) == 0x1)
		{
			i++;
			udelay(10000);
			if(i == 150)
			{
				...
				setenv("bootcmd","movi read kernel C0008000; movi read covery 40A00000 300000; bootm C0008000 40A00000");
				return 0;
			}
		}
#if 0
		...
		int keymask[8];
		int cval = 0,rval = 0 ,col; 
		writel((readl(ELFIN_GPIO_BASE+GPH2CON_OFFSET)&(~0xFFFFFFFF))|0x11111111, ELFIN_GPIO_BASE+GPH2CON_OFFSET);
		writel(readl(ELFIN_GPIO_BASE+GPH2PUD_OFFSET)&(~0xFFFF), ELFIN_GPIO_BASE+GPH2PUD_OFFSET);
		writel(readl(ELFIN_GPIO_BASE+GPH3CON_OFFSET)&(~0xFFFFFFFF), ELFIN_GPIO_BASE+GPH3CON_OFFSET);
		writel((readl(ELFIN_GPIO_BASE+GPH3PUD_OFFSET)&(~0xFFFF))|0xAAAA, ELFIN_GPIO_BASE+GPH3PUD_OFFSET);

		/*the GPH3DAT 1~2 data is not correct, so read twice*/
		gpio_value = readl(ELFIN_GPIO_BASE+GPH3DAT_OFFSET)&0xFF;
		gpio_value = readl(ELFIN_GPIO_BASE+GPH3DAT_OFFSET)&0xFF;
		while((readl(ELFIN_GPIO_BASE+GPH3DAT_OFFSET)&0xFF) != 0xBF)
		{
			for(col =0; col< 8; col++)
			{
				/* clear that column number and make that normal output */
