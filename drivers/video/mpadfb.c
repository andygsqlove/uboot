...
static void Disp_set_syscon_clk_fimd(void)
{
	...
	//閫夋嫨FIMD鐨勬椂閽熸簮涓篠CLKMPLL=667MHz
	cfg = CLK_SRC1_REG & ~(0xf<<20);
	CLK_SRC1_REG = cfg | (0x6<<20);
	
	//璁剧疆FIMD鐨勫垎棰戠郴鏁帮紝SCLK_FIMD = MOUTFIMD / (FIMD_RATIO + 1)锛屽墠闈㈠凡缁忛�夋嫨MOUTFIMD=SCLKMPLL锛屽嵆SCLK_FIMD=SCLKMPLL/(FIMD_RATIO+1)
...
void lcd_port_init()
{
...
#if 0	
	 //set gpd0_3 (backlight pwm pin) output and pull up and high level for disabled
	writel((readl(GPD0CON) & ~(0xf<<12)) | (0x1<<12), GPD0CON);
	writel((readl(GPD0PUD) & ~(0x3<<6)) | (0x2<<6), GPD0PUD);
	writel((readl(GPD0DAT) & ~(0x1<<3)) | (0x1<<3), GPD0DAT);
	...
	writel((readl(GPH1DAT) & ~(0x1<<7)) | (0x0<<7), GPH1DAT);
	 ...
	 //set gpj_5 (backlight enable pin) output and pull up and low level for disabled
	writel((readl(GPJ1CON) & ~(0xf<<20)) | (0x1<<20), GPJ1CON);
	writel((readl(GPJ1PUD) & ~(0x3<<10)) | (0x2<<10), GPJ1PUD);
	writel((readl(GPJ1DAT) & ~(0x1<<5)) | (0x0<<5), GPJ1DAT);

//	printf("======GPJ1DAT=%x\n",readl(GPJ1DAT));

	//LCD_BL_EN EINT24  GPH3_0  hight level for enabled ok  需要输出高电平
	 //set gpj_5 (backlight enable pin) output and pull up and low level for disabled
	writel((readl(GPH3CON) & ~(0xf<<0)) | (0x1<<0), GPH3CON);
	writel((readl(GPH3PUD) & ~(0x3<<0)) | (0x2<<0), GPH3PUD);
	writel((readl(GPH3DAT) & ~(0x1<<0)) | (0x1<<0), GPH3DAT);

	//LCD_EN EINT26 GPH3_2 hight level for enabled  ok 从芯片上看，这个脚输出高电平有效
	writel((readl(GPH3CON) & ~(0xf<<8)) | (0x1<<8), GPH3CON);
	writel((readl(GPH3PUD) & ~(0x3<<4)) | (0x2<<4), GPH3PUD);
	writel((readl(GPH3DAT) & ~(0x1<<2)) | (0x1<<2), GPH3DAT);

	//LVDS_TRAN_EN    GPJ1_5  输出高
	writel((readl(GPJ1CON) & ~(0xf<<20)) | (0x1<<20), GPJ1CON);
	writel((readl(GPJ1PUD) & ~(0x3<<10)) | (0x2<<10), GPJ1PUD);
	writel((readl(GPJ1DAT) & ~(0x1<<5)) | (0x1<<5), GPJ1DAT);
...
void backlight_brigness_init(u8 brightness)
{
	if(brightness)
	{
		printf("backlight_brigness_init\n");
...
#else//AT070TN92
	backlight_brigness_init(1);  //xjm mask
#endif
